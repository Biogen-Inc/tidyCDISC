...
# disp_type = "warn"
gather_reqs <- function(disp_type,
  ...
  hdf <- rbindlist(hdfs, fill=TRUE, idcol = "df")
  # hdf # peek
  ...
  dfw <- lapply(df_incl_rules, data.frame, stringsAsFactors = FALSE) %>%
    rbindlist( fill=TRUE, idcol = "df_var")
  # dfw # peek
...
dataComply <- function(input, output, session,
  ...
  observeEvent(datalist(), {
    ...
    if(nrow(err_tab$df) > 0){
      showModal( 
         modalDialog(footer = NULL, glue("Error!")
      ))
    }
    else if(nrow(wrn_tab$df)){
      showModal( 
         modalDialog(glue("Warning!")
      ))
    }
