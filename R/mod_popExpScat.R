#' popExpScat Server Function
#' 
#' Generate a Scatter plot.
#' Child of parent module mod_popExp.R.
#' Calls: fnoverplt and fnscatter.
#'
...
#' @import shiny
#' @import dplyr
#' @importFrom stringr str_detect str_replace str_locate str_c str_length
...
#' @noRd
#' 
mod_popExpScat_server <- function(input, output, session, df){
  ns <- session$ns
  ...
  # update selectxvar and selectyvar based on PARAM code selection
  observeEvent(input$selPrmCode, {
    ...
    if (str_detect(prmsel,",") == FALSE) {
      
      # print(paste("just one PARAMCD:",prmsel))
      dfsub <<- filter(dfsel,PARAMCD %in% input$selPrmCode) # superassignment operator
      
    } else {
      # if two PARAMCDs selected, then we need to use pivot_wider()
      
      # print(paste("two PARAMCDS:",prmsel))
      suppressWarnings(req(dfsub$AVAL)) # AVAL needs to exist.
      
      dftwo <- filter(dfsel,PARAMCD %in% input$selPrmCode) # dfsel created from df() above
      
      dfsubw <- dftwo %>%
        pivot_wider(id_cols=c(USUBJID, AVISIT), names_from = c(PARAMCD), values_from = c(AVAL, BASE, CHG), names_sep = "_") 
      
      dfmrg <- dfsub[, !names(dftwo) %in% c(names(dfsubw),"PARAM","PARAMCD","AVAL","BASE","CHG")]
      
      # add USUBJID and AVISIT
      dfmrg2 <- bind_cols(select(dfsub,USUBJID,AVISIT),dfmrg) %>%
        distinct(USUBJID, AVISIT, .keep_all = TRUE)
      
      dfsub <<- left_join(dfsubw, dfmrg2, by = c("USUBJID","AVISIT")) %>%  # superassignment operator
    ...
    updateSelectInput(session = session, inputId = "selxvar", choices = c("",sort(names(dfsub))),  selected=character(0))
    updateSelectInput(session = session, inputId = "selyvar", choices = c("",sort(names(dfsub))),  selected=character(0))
    
  }, ignoreInit = TRUE, ignoreNULL = FALSE) # observeEvent(input$selPrmCode
  ...
  output$PlotlyOut <- renderPlotly({
    ...
    req(input$selPrmCode != "") 
    # print(paste("is dfsub NULL?",is.null(dfsub)))
    req(!is.null(dfsub))
    ...
    if (str_detect(unique(dfsub$PARAMCD),"_") == TRUE 
        && !is.na(str_locate(xvar,"_")[1]) && !is.na(str_locate(yvar,"_")[1]))  {
    ...
    if (input$DiscrXaxis == TRUE) {
      p <- p + scale_x_discrete(limits=c(sort(unique(dfsub[[v$selxvar]]))))
    }
  ...
  output$DataTable <- DT::renderDataTable({
    ...
    if(input$groupbox == TRUE) {
      ...
      if ("USUBJID" %in% colnames(dfsub)) {
        tableout <- dfsub %>%
          dplyr::select(USUBJID, !!sym(input$groupbyvar), !!sym(v$selxvar), !!sym(v$selyvar)) 
      } else {
        tableout <- dfsub %>%
          dplyr::select(!!sym(input$groupbyvar), !!sym(v$selxvar), !!sym(v$selyvar))
      }
    } else {
      if ("USUBJID" %in% colnames(dfsub)) {
        tableout <- dfsub %>%
          dplyr::select(USUBJID, !!sym(v$selxvar), !!sym(v$selyvar))
      } else {
        tableout <- dfsub %>%
          dplyr::select(!!sym(v$selxvar), !!sym(v$selyvar))
      }
    }
...
# mod_popExpScat_ui("popExpScat_ui_1")
    
## To be copied in the server - done
# callModule(mod_popExpScat_server, "popExpScat_ui_1")
