...
#'
#' Gather various shiny inputs from OCCDs datasets (if they exist) and return a single standardized events dataset
#'
#' @param input_checkbox A Character vector of Events Selected
#' @param input_apply_filter If \code{TRUE} then toggle dataset used to build events
#' @param my_loaded_adams A character vector of available dataframes in my_datafile
#' @param my_datafile A list of data frames
#' @param my_filtered_dat The output from IDEA- a filtered dataset 
#' @param my_usubjid The patient number, in the standard USUBJID format
#'
#'   DO NOT REMOVE.
#' @import dplyr
...
build_events <- function(
  ...
  if ("ADAE" %in% my_loaded_adams & "AE" %in% c(input_checkbox)) { # ac: first part not needed?
  ...
  if ("ADLB" %in% my_loaded_adams & "LB" %in% c(input_checkbox)) {
    if("LBDT" %in% colnames(my_datafile[["ADLB"]])){
      lb_rec <- (if(input_apply_filter == T) my_datafile[["ADLB"]] %>% semi_join(my_filtered_dat) else my_datafile[["ADLB"]]) %>%
        filter(USUBJID == my_usubjid) %>%
  ...
  if ("ADMH" %in% my_loaded_adams & "MH_" %in% substring(input_checkbox, 1, 3)) {
    ...
    if("MHSTDTC" %in% colnames(my_datafile[["ADMH"]])){
      mh_rec <- (if(input_apply_filter == T) my_datafile[["ADMH"]] %>% semi_join(my_filtered_dat) else my_datafile[["ADMH"]]) %>%
        filter(USUBJID == my_usubjid) %>%
