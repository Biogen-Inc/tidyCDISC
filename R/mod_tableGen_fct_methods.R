...
CDISC_methods <- function(agg, column, week, group, data) {
  if (agg == "MEAN") {
    CDISC_mean(column, week, group, data)
  } else if (agg == "FREQ") {
    CDISC_freq(column, week, group, data)
  } else if (agg == "ANOVA") {
    CDISC_anova(column, week, group, data)
  } else {
    CDISC_chg(column, week, group, data)
  }
}
