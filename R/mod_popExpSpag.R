#' popExpSpag Server Function
#' 
#' Generate a Spaghetti plot.
#' Child of parent module mod_popExp.R.
#'
#' @param input,output,session Internal parameters for {shiny}. 
#' @param df dataframe passed to mod_popExpSpag.
#' 
#'   DO NOT REMOVE.
#' @import shiny
#' @import dplyr
...
#' @noRd
#' 
mod_popExpSpag_server <- function(input, output, session, df){
  ns <- session$ns
  ...
  output$PlotlyOut <- renderPlotly({
    ...
    if (input$animate == TRUE) {
      ...
      ggplotly(p, tooltip = "text")  %>%
        ...
        plotly::layout( legend = list(orientation = "h",  xanchor = "center", yanchor = "bottom", x=0.5, y=-0.35),
                title = ggtitle())
...
# mod_popExpSpag_ui("popExpSpag_ui_1")
    
## To be copied in the server -- done
# callModule(mod_popExpSpag_server, "popExpSpag_ui_1")
