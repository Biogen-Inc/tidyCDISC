...
However, the date range displayed is subject to where the user wishes to zoom & pan. The user can either "hover and scroll" or leverage the small `+` or `-` sign on the top-right corner of the plot to zoom. To pan, you can click and drag the timeline plot's screen to the left or right. As you zoom & pan, you'll see more and more detail in the x-axis and the events plotted will begin to spread out. See animation below:
...
First: the "Select a loaded ADaM with columns 'PARAM' & 'AVAL'" input is hinting that only data sets of the BDS-class format will populate the drop-down. Notice that the lab and vital signs data meet these requirements. Once a data set is selected, we'll use its data for plotting. We'll continue on with the `ADLBC` data set.
