...
- Fixed filtering module in Population Explorer where columns for non-selected datasets would erroneously display

- Added an app.R file so that the Shiny app can be run directly from `shiny::runGitHub()` or `runUrl()`

- Table title automatically when a "standard analysis" is selected
...
So we want to hear from you! Does `tidyCDISC` not work for you? then [tell us why.](https://github.com/Biogen-Inc/tidyCDISC/issues) Is there something different about the way you (or your company) handles certain tables / analyses? We want to know about it. Do you have a passion to see clinical trials analysis in R come to fruition in the next few years? Help us shape the process. Do you want to advance the industry forward by contributing to an open source project? Become a `tidyCDISC` developer!
