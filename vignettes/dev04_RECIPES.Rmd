...
This JSON object when included in `recipes.json` will create a table for mean Diastolic Blood Pressure at all Time Points and all Visits:
```{r, eval=FALSE}
"tbl_1": {
  "title": "Custom Table 1: Diastolic Blood Pressure by Visit",
  "blocks": [{
    "data":"ADVS",
    "variable":"DIABP",
    "var_options_fn":"atpt",
...
Here are the functions to create the variable options and the statistic options (note that these functions are already included in the package):
```{r, eval=FALSE}
...
var_options.atpt <- function(block, datalist, ...) {
  atpts <-
    ...
    dplyr::pull(ATPT) %>%
    get_levels() %>%
    {list(ATPT = as.list(.))} # Note that this is a named list
...
```{r, echo=FALSE}
knitr::asis_output(htmltools::includeHTML("figures/recipes/ATPT_tbl.html"))
```