...
```{r, out.width="100%", fig.align='center', out.extra = 'style="border:1px solid; padding:5px;"', echo=FALSE}
...
A few other elements can also be present within a `block` object, such as: `stat_arg`, `stat_options`, and `stat_options_fn`.  A `stat_arg` element is only required when the `statistic` element requires it. As seen in the example above, the "NESTED_FREQ_ABC" `statistic` needs to know a secondary variable by which to produce its "nested" output. Besides those mentioned, no other elements are used in the `recipes.json` file or within the application's code base, but active development will be changing this soon.
