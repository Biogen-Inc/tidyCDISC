...
Any uploaded data sets will appear on the left-hand side as "bins". "Bins" are just containers, showcasing the usable contents of the data when expanded. The `ADSL` & `ADAE` bins include the names of all their variables, while any `BDS` data sets, like the `ADVS` & `ADLBC` bins include a list of all the `PARAMCD` values.
...
In order to build a table, we need only drag a variable block to the "Variable" drop zone and match it up with a corresponding stat block in the "Stats" drop zone as we demonstrate in the following example:

<br>

### Age Summary Statistics

We can drag the `AGE` block from within `ADSL` and drag in the `MEAN` block from the available Stats to calculate summary statistics for patient `AGE` within the trial.
